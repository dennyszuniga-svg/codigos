...
language sql security definer set search_path = public as $$
  ...
  where p.activo
    and (
      ...
      or (p.rol = 'encargado_ti' and p.id = auth.uid() and p.sede = sede_arg)
    )
    and (
      public.es_operacion_global()
      or exists (
        select 1 from public.profiles a
        where a.id = auth.uid() and a.activo and a.rol = 'admin' and a.sede = sede_arg
      )
    )
...
declare
  item jsonb; total integer := 0; sede_item text; user_item uuid;
  fecha_item date; turno_item uuid; estado_item text;
begin
  ...
  for item in select * from jsonb_array_elements(items_arg) loop
    sede_item := item->>'sede'; user_item := (item->>'user_id')::uuid;
    fecha_item := (item->>'fecha')::date; turno_item := nullif(item->>'turno_id', '')::uuid;
    estado_item := coalesce(item->>'estado', 'programado');
    if not (
      public.es_operacion_global()
      or exists (select 1 from public.profiles p where p.id = auth.uid() and p.activo and p.rol = 'admin' and p.sede = sede_item)
    ) then raise exception 'No autorizado'; end if;
    if not exists (
      select 1 from public.profiles p
      where p.id = user_item and p.activo
        and (
          (p.rol in ('anfitrion', 'tecnico', 'supervisor', 'fortaleza') and (p.sede = sede_item or p.rol = 'tecnico'))
          or (p.rol = 'encargado_ti' and p.id = auth.uid() and p.sede = sede_item)
        )
    ) then raise exception 'Trabajador no valido para la sede'; end if;
    ...
    on conflict(user_id, fecha) do update set sede = excluded.sede, turno_id = excluded.turno_id,
...
begin
  ...
  return query
  select p.id, p.nombre, p.rol, coalesce(sum(r.minutos_trabajados), 0)::bigint,
    round(coalesce(sum(r.minutos_trabajados), 0)::numeric / 60, 2), count(r.id)::bigint,
    ...
    coalesce(sum(r.horas_extra_25), 0)::bigint, coalesce(sum(r.horas_extra_35), 0)::bigint,
  ...
  where p.activo
    and (p.rol in ('anfitrion', 'tecnico', 'supervisor', 'fortaleza') or (p.rol = 'encargado_ti' and p.sede = sede_arg))
    and (p.sede = sede_arg or p.rol = 'tecnico')
