...
set search_path = public as $$
  select nivel = 'todos' or exists (
    ...
    where id = auth.uid()
      and activo = true
      and rol in ('admin', 'supervisor', 'fortaleza')
  );
$$;
...
set search_path = public as $$
  ...
  where p.activo
    and p.rol in ('anfitrion', 'tecnico', 'supervisor', 'fortaleza')
    and (
...
grant execute on function public.es_supervision() to authenticated;
grant execute on function public.puede_ver_guia(text) to authenticated;
