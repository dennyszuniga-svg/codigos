...
add column if not exists costo_unitario_sin_igv numeric(14,4) not null default 0
check (costo_unitario_sin_igv >= 0);

alter table public.movimientos_stock_repuestos
add column if not exists costo_unitario_sin_igv numeric(14,4) not null default 0,
add column if not exists costo_total_sin_igv numeric(14,2) not null default 0,
add column if not exists costo_total_con_igv numeric(14,2) not null default 0,
add column if not exists sede_consumo text,
add column if not exists equipo_detalle text;

drop function if exists public.guardar_stock_repuesto_multiples(text,text,text,text,text,text,text,text[],numeric,numeric,text);
...
create or replace function public.guardar_stock_repuesto_multiples(
    ...
    costo_unitario_arg numeric, compatibilidad_arg text, proveedor_arg text, contacto_arg text,
...
begin
    ...
    if stock_arg < 0 or stock_minimo_arg < 0 or costo_unitario_arg < 0 then raise exception 'Stock o costo no valido'; end if;

    insert into public.catalogo_repuestos (codigo,nombre,categoria,unidad,costo_unitario_sin_igv,compatibilidad,proveedor,contacto_proveedor,creado_por)
    values (upper(trim(codigo_arg)),trim(nombre_arg),coalesce(nullif(trim(categoria_arg),''),'General'),coalesce(nullif(trim(unidad_arg),''),'unidad'),
      costo_unitario_arg,compatibilidad_arg,nullif(trim(proveedor_arg),''),nullif(trim(contacto_arg),''),auth.uid())
    on conflict (codigo) do update set nombre=excluded.nombre,categoria=excluded.categoria,unidad=excluded.unidad,
      costo_unitario_sin_igv=excluded.costo_unitario_sin_igv,compatibilidad=excluded.compatibilidad,
      proveedor=excluded.proveedor,contacto_proveedor=excluded.contacto_proveedor
...
drop function if exists public.registrar_movimiento_stock(uuid,text,numeric,text,text);
drop function if exists public.registrar_movimiento_stock(uuid,text,numeric,text,text,text,text);
...
begin
    ...
    select c.costo_unitario_sin_igv into costo_unitario from public.catalogo_repuestos c where c.id=origen.repuesto_id;
    ...
    insert into public.movimientos_stock_repuestos(
      repuesto_id,stock_origen_id,ubicacion_origen,ubicacion_destino,tipo,cantidad,stock_anterior,stock_resultante,
      costo_unitario_sin_igv,costo_total_sin_igv,costo_total_con_igv,sede_consumo,equipo_detalle,observacion,creado_por)
    values(origen.repuesto_id,origen.id,origen.ubicacion_sede,destino_arg,tipo_arg,cantidad_arg,origen.stock,nuevo,
      costo_unitario,costo_sin_igv,round(costo_sin_igv*1.18,2),sede_consumo_arg,nullif(trim(equipo_arg),''),left(coalesce(observacion_arg,''),500),auth.uid());
    return nuevo;
end; $$;
...
returns table(id uuid,repuesto_id uuid,codigo text,nombre text,categoria text,unidad text,compatibilidad text,proveedor text,
 contacto_proveedor text,costo_unitario_sin_igv numeric,ubicacion_sede text,stock numeric,stock_minimo numeric,ubicacion_detalle text,stock_total numeric)
language sql security definer set search_path=public as $$
 select s.id,c.id,c.codigo,c.nombre,c.categoria,c.unidad,c.compatibilidad,c.proveedor,c.contacto_proveedor,c.costo_unitario_sin_igv,
...
begin
 ...
 for item in select * from jsonb_array_elements(repuestos_arg) loop
   ...
   insert into public.movimientos_stock_repuestos(
     repuesto_id,stock_origen_id,ubicacion_origen,ubicacion_destino,tipo,cantidad,stock_anterior,stock_resultante,numero_informe,
     costo_unitario_sin_igv,costo_total_sin_igv,costo_total_con_igv,observacion,creado_por)
   values(origen.repuesto_id,origen.id,origen.ubicacion_sede,sede_arg,'consumo',cantidad_num,origen.stock,origen.stock-cantidad_num,numero_informe_arg,
     costo_unitario,costo_sin_igv,round(costo_sin_igv*1.18,2),coalesce(item->>'observacion','Consumo desde informe'),auth.uid());
 end loop;
end; $$;

create or replace function public.actualizar_costo_repuesto(repuesto_id_arg uuid,costo_arg numeric)
returns void language plpgsql security definer set search_path=public as $$
begin
 if not public.es_encargado_ti() then raise exception 'No autorizado'; end if;
 if costo_arg < 0 then raise exception 'Costo no valido'; end if;
 update public.catalogo_repuestos set costo_unitario_sin_igv=costo_arg where id=repuesto_id_arg;
...
returns table(
 fecha timestamptz,tipo text,numero_informe text,sede_consumo text,equipo_codigo text,equipo_nombre text,
 codigo text,repuesto text,cantidad numeric,unidad text,costo_unitario_sin_igv numeric,
 costo_total_sin_igv numeric,igv numeric,costo_total_con_igv numeric,observacion text
)
...
begin
 ...
 return query
 select m.created_at,m.tipo,m.numero_informe,
   ...
   coalesce(i.equipo_codigo,m.equipo_detalle),i.equipo_nombre,c.codigo,c.nombre,m.cantidad,c.unidad,m.costo_unitario_sin_igv,
 ...
 where m.tipo in('consumo','salida') and m.created_at>=inicio and m.created_at<(inicio+interval '1 month')
 order by m.created_at desc;
end; $$;

grant execute on function public.guardar_stock_repuesto_multiples(text,text,text,text,numeric,text,text,text,text[],numeric,numeric,text) to authenticated;
grant execute on function public.registrar_movimiento_stock(uuid,text,numeric,text,text,text,text) to authenticated;
...
grant execute on function public.actualizar_costo_repuesto(uuid,numeric) to authenticated;
